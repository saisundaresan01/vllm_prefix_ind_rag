...
class GPUModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    ...
    def __init__(
        ...
        self.positions = self._make_buffer(self.max_num_tokens,
                                           dtype=torch.int64)
        self.prefix_ind_positions = self._make_buffer(self.max_num_tokens,
                                                dtype=torch.int64)
    ...
    def _calc_prefix_ind_offsets(self, scheduler_output: "SchedulerOutput", cumsum_dtype: Optional[np.dtype] = None):
        prefix_ind_pos_ptr = 0
        new_id_to_req_map = {}

        for req in scheduler_output.scheduled_new_reqs:
            new_id_to_req_map[req.req_id] = req

        for index, req_id in enumerate(self.input_batch.req_ids):
            if req_id in new_id_to_req_map:
                req = new_id_to_req_map[req_id]
                chunk_lens = req.padded_chunk_lens
                chunk_sequence = req.chunk_sequence
                num_scheduled_tokens = scheduler_output.num_scheduled_tokens[req_id]
                num_computed_tokens = req.num_computed_tokens

                cu_num_tokens = np.cumsum(chunk_lens, dtype=cumsum_dtype)
                total_num_tokens = cu_num_tokens[-1]
                ...
                for l, s in zip(chunk_lens, chunk_sequence):
                    if s != cur_seq_idx:
                        cur_seq_idx = s
                        prev_max_pos = max_pos
                    chunk_start_pos.append(prev_max_pos)
                    max_pos = max(max_pos, prev_max_pos + l)

                chunk_start_pos = np.array(chunk_start_pos)
                cumsums_offsets = np.repeat(cu_num_tokens - chunk_lens - chunk_start_pos, chunk_lens)
                arange = self.arange_np[:total_num_tokens] - cumsums_offsets
                
                dst_start = prefix_ind_pos_ptr
                dst_end = prefix_ind_pos_ptr + num_scheduled_tokens
                src_start = num_computed_tokens
                src_end = num_computed_tokens + num_scheduled_tokens

                self.prefix_ind_positions.np[dst_start:dst_end] = arange[src_start:src_end]
                self.input_batch.max_prefix_ind_positions_cpu[index] = max_pos
                prefix_ind_pos_ptr += num_scheduled_tokens
            else:
                num_scheduled_tokens = scheduler_output.num_scheduled_tokens[req_id]
                dst_start = prefix_ind_pos_ptr
                dst_end = prefix_ind_pos_ptr + num_scheduled_tokens
                self.prefix_ind_positions.np[dst_start:dst_end] = (self.input_batch.max_prefix_ind_positions_cpu[index] + self.arange_np[:num_scheduled_tokens])
                prefix_ind_pos_ptr += num_scheduled_tokens
                self.input_batch.max_prefix_ind_positions_cpu[index] += num_scheduled_tokens
    ...
    def _prepare_inputs(
        ...
        self._calc_prefix_ind_offsets(scheduler_output)
        ...
        if self.uses_mrope:
            ...
            self.prefix_ind_positions.copy_to_gpu(total_num_scheduled_tokens)
    ...
    def _preprocess(
        ...
        if self.uses_mrope:
            ...
            positions = self.prefix_ind_positions.gpu[:num_input_tokens]
